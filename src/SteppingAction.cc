...
void SteppingAction::UserSteppingAction(const G4Step* aStep) {
  ...
  if (aStep->GetPreStepPoint()->GetStepStatus() != fWorldBoundary &&
    ...
    if (PreHitPos.z() > 55.495 * mm)  // Now surface is staggered!!
                                      //...why again was step Within error
                                      //bound.. what is MAX step size??
    {
      if (ThisHitPos.y() > -42.999 * mm &&
          ThisHitPos.y() < 39.999 * mm)  // High to low check or vv is faster?
      {
        if ((ThisHitPos.x() > -39.999 * mm && ThisHitPos.x() < -19.2495 * mm &&
             ThisHitPos.z() < 55.505 * mm) ||
            (ThisHitPos.x() > -19.2495 * mm && ThisHitPos.x() < -1.500 * mm &&
             ThisHitPos.z() < 56.63 * mm) ||
            (ThisHitPos.x() > -1.500 * mm && ThisHitPos.x() < 22.2495 * mm &&
             ThisHitPos.z() < 57.755 * mm) ||
            (ThisHitPos.x() > 22.2495 * mm && ThisHitPos.x() < 42.999 * mm &&
             ThisHitPos.z() < 58.88 * mm))

        {
          if (postvolume->GetName()) {
            G4String postname = postvolume->GetName();

            if (postname == det1 || postname == det2 || postname == det3 ||
                postname == det4 || postname == det5 || postname == det6 ||
                postname == det7 || postname == det8 || postname == det9 ||
                postname == det10 || postname == det11 || postname == det12 ||
                postname == det13 || postname == det14 || postname == det15 ||
                postname == det16) {

              histo->FillPositionResolutionTruth(ThisHitPos.x(), ThisHitPos.y(),
                                                 ThisHitPos.z(), totalE, pnum);
            }  // end of det checks
    ...
    if (PreHitPos.z() > 59.99 && PreMaterial->GetName() == proQuartz &&
        eDeposit > 0.01 * eV) {
      //	  G4cout<<"The particle was "<<pname<<" and its energy deposit
      //was "<<eDeposit<<"and it hit "<<volume->GetName()<<G4endl;
      if (name == det1)
        histo->FillRPDHitEnergies(eDeposit, pnum, 0);  // RPD1 Hits
      else if (name == det2)
        histo->FillRPDHitEnergies(eDeposit, pnum, 1);  // RPD2 Hits
      else if (name == det3)
        histo->FillRPDHitEnergies(eDeposit, pnum, 2);  // RPD3 Hits
      else if (name == det4)
        histo->FillRPDHitEnergies(eDeposit, pnum, 3);  // RPD4 Hits
      else if (name == det5)
        histo->FillRPDHitEnergies(eDeposit, pnum, 4);  // RPD5 Hits
      else if (name == det6)
        histo->FillRPDHitEnergies(eDeposit, pnum, 5);  // RPD6 Hits
      else if (name == det7)
        histo->FillRPDHitEnergies(eDeposit, pnum, 6);  // RPD7 Hits
      else if (name == det8)
        histo->FillRPDHitEnergies(eDeposit, pnum, 7);  // RPD8 Hits
      else if (name == det9)
        histo->FillRPDHitEnergies(eDeposit, pnum, 8);  // RPD9 Hits
      else if (name == det10)
        histo->FillRPDHitEnergies(eDeposit, pnum, 9);  // RPD10 Hits
      else if (name == det11)
        histo->FillRPDHitEnergies(eDeposit, pnum, 10);  // RPD11 Hits
      else if (name == det12)
        histo->FillRPDHitEnergies(eDeposit, pnum, 11);  // RPD12 Hits
      else if (name == det13)
        histo->FillRPDHitEnergies(eDeposit, pnum, 12);  // RPD13 Hits
      else if (name == det14)
        histo->FillRPDHitEnergies(eDeposit, pnum, 13);  // RPD14 Hits
      else if (name == det15)
        histo->FillRPDHitEnergies(eDeposit, pnum, 14);  // RPD15 Hits
      else if (name == det16)
        histo->FillRPDHitEnergies(eDeposit, pnum, 15);  // RPD16 Hits
    }

    G4cout<<"optical photon with energy = "<< eDeposit<< "MeV"<<G4endl;

    //	Edep(2:6eV)
    if (pname = "opticalphoton" && PostMaterial->GetName() == pmtmat &&
                eDeposit > 0.001 * eV && eDeposit < 7.0 * eV)

    {
      if (postvolume->GetName() == pmt1) {
        histo->FillPMTHitEnergies(eDeposit, 0);
        histo->FillPMTChannelEnergy(0);
      }  // RPD1 Hits

      else if (postvolume->GetName() == pmt2) {
        histo->FillPMTHitEnergies(eDeposit, 1);
        histo->FillPMTChannelEnergy(1);
      }  // RPD2 Hits

      else if (postvolume->GetName() == pmt3) {
        histo->FillPMTHitEnergies(eDeposit, 2);
        histo->FillPMTChannelEnergy(2);
      }
      G4cout<<"pmt3 photon with energy = "<<totalE<<G4endl;

      else if (postvolume->GetName() == pmt4) {
        histo->FillPMTHitEnergies(eDeposit, 3);
        histo->FillPMTChannelEnergy(3);
      }  // RPD4 Hits

      else if (postvolume->GetName() == pmt5) {
        histo->FillPMTHitEnergies(eDeposit, 4);
        histo->FillPMTChannelEnergy(4);
      }  // RPD5 Hits

      else if (postvolume->GetName() == pmt6) {
        histo->FillPMTHitEnergies(eDeposit, 5);
        histo->FillPMTChannelEnergy(5);
      }  // RPD6 Hits

      else if (postvolume->GetName() == pmt7) {
        histo->FillPMTHitEnergies(eDeposit, 6);
        histo->FillPMTChannelEnergy(6);
      }
      G4cout<<"pmt7 photon with energy = "<<totalE<<G4endl;

      else if (postvolume->GetName() == pmt8) {
        histo->FillPMTHitEnergies(eDeposit, 7);
        histo->FillPMTChannelEnergy(7);
      }  // RPD8 Hits

      else if (postvolume->GetName() == pmt9) {
        histo->FillPMTHitEnergies(eDeposit, 8);
        histo->FillPMTChannelEnergy(8);
      }  // RPD9 Hits

      else if (postvolume->GetName() == pmt10) {
        histo->FillPMTHitEnergies(eDeposit, 9);
        histo->FillPMTChannelEnergy(9);
      }  // RPD10 Hits

      else if (postvolume->GetName() == pmt11) {
        histo->FillPMTHitEnergies(eDeposit, 10);
        histo->FillPMTChannelEnergy(10);
      }
      G4cout<<"pmt11 photon with energy = "<<totalE<<G4endl;

      else if (postvolume->GetName() == pmt12) {
        histo->FillPMTHitEnergies(eDeposit, 11);
        histo->FillPMTChannelEnergy(11);
      }  // RPD12 Hits

      else if (postvolume->GetName() == pmt13) {
        histo->FillPMTHitEnergies(eDeposit, 12);
        histo->FillPMTChannelEnergy(12);
      }  // RPD13 Hits

      else if (postvolume->GetName() == pmt14) {
        histo->FillPMTHitEnergies(eDeposit, 13);
        histo->FillPMTChannelEnergy(13);
      }  // RPD14 Hits

      else if (postvolume->GetName() == pmt15) {
        histo->FillPMTHitEnergies(eDeposit, 14);
        histo->FillPMTChannelEnergy(14);
      }
      G4cout<<"pmt15 photon with energy = "<<totalE<<G4endl;

      else if (postvolume->GetName() == pmt16) {
        histo->FillPMTHitEnergies(eDeposit, 15);
        histo->FillPMTChannelEnergy(15);
      }  // RPD16 Hits
    }
