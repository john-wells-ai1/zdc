...
Analysis::Analysis() : myDet(0) {
  zoffset = 0.;
  energy = 10. * GeV;

  TH1::StatOverflows(
      kTRUE);  // include overflow and under flow in mean/rms calculations
}
...
void Analysis::Init() {
  ...
  // event observables
  histos["lProfile"] =
      new TH1D("lProfile", "longitudinal profile", 100, zmin, zmax);
  histos["rProfile"] = new TH1D("rProfile", "radial profile", 100, rmin, rmax);

  // run oservables
  histos["visEne"] =
      new TH1D("visEne", "total visible Energy", 100, emin, emax);
  histos["visEne1"] = new TH1D("visEne1", "total visible Energy in adsorber 1",
                               100, emin, emax);
  histos["visEne2"] = new TH1D("visEne2", "total visible Energy in adsorber 2",
                               100, emin, emax);
}
...
void Analysis::BeginOfRun(G4int n) { Init(); }
...
void Analysis::EndOfEvent(G4VHitsCollection* hitCollection)  // the analysis
{
  ...
  for (size_t i = 0; i < shc->GetSize(); ++i) {
    ...
    vis += sh->GetEdep();
    G4cout << "Got a hit collection, got the position of hit: " << pos
           << G4endl;
  }
  visEne->Fill(vis);
  visEne1->Fill(vis1);
