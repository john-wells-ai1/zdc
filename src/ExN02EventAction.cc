...
void ExN02EventAction::EndOfEventAction(const G4Event* evt) {
  ...
  if (event_id < 100 || event_id % 100 == 0) {
    G4cout << ">>> Event " << evt->GetEventID() << " has " << n_trajectories
  ...
  for (int intCell = 0; intCell < 16; ++intCell) {
    ...
    if (hitsCollections) {
      ...
      hits =
          static_cast<CaloHitCollection*>(hitsCollections->GetHC(hitsCollID));
    }
  ...
  for (int intCell = 0; intCell < 16; ++intCell) {
    PhotonDet[intCell] =
        "PhotonDet" + G4UIcommand::ConvertToString(intCell + 1);
  ...
  myana->FillChannelEnergy(Erpd[0], Erpd[1], Erpd[2], Erpd[3], Erpd[4], Erpd[5],
                           Erpd[6], Erpd[7], Erpd[8], Erpd[9], Erpd[10],
                           Erpd[11], Erpd[12], Erpd[13], Erpd[14], Erpd[15]);
  ...
  G4cout << "EndOfEventAction: primary position: " << pos
         << ", primary momentum: " << mom << G4endl;
}
