...
CalorimeterSD::CalorimeterSD(G4String SDname) : G4VSensitiveDetector(SDname) {
...
G4bool CalorimeterSD::ProcessHits(G4Step* step, G4TouchableHistory*) {
  // step is guaranteed to be in Scintillator volume : no need to check for
  // volume
...
void CalorimeterSD::Initialize(G4HCofThisEvent* HCE) {
  ...
  std::cout << "CalorimeterSD::Initialize: create new hitcollection "
            << GetName() << " with collection name: " << collectionName[0]
            << std::endl;

  // ----------------------------------------------------------------------------
  // -- and attachment of this collection to the "Hits Collection of this
  // Event":
  // ----------------------------------------------------------------------------
  ...
  hitCollection =
      new CaloHitCollection(SensitiveDetectorName, collectionName[0]);
  if (HCID < 0) {
...
void CalorimeterSD::EndOfEvent(G4HCofThisEvent* HE) {
  // test output of hits
    G4cout << "CalorimeterSD::EndOfEvent method of SD `" << GetName() << "'
    called." << G4endl;
